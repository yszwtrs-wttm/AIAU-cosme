...
| 口コミ信頼度 | `recompute_review_trust`。スコアは内部だけで使い、除外理由は「見極めモード」の開閉式で言葉にして見せる |
...
通常表示は「集計に使った口コミ n 件 / 参考にしなかった m 件」だけにとどめ、外した投稿とその理由は商品ページの「見極めモード」（`<details>` の開閉式）に畳む。文言は `src/lib/wording.ts` の `excludeReasonText` で「同じような文面の投稿が複数見つかりました」のように、サクラだと断定しない言い方に統一する。
