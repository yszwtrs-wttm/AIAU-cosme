...
`npm run db:reset` を実行すると、デモ用アカウントとポーチ5点まで入る。商品IDも固定なので、下の順番で
URL を開けば必ず同じ画面になる。

- デモ用アカウント: `demo@kawanai.test` / パスワード `kawanai-demo`
- ポーチ（5点）: LUMINA グロウリップスティック 03 / DAILY+ うるおいリップ 02 / LUMINA スキンフィットファンデーション 200 / LUMINA デイリーアイパレット 01 / GRIT スカルプシャンプー クール

| # | URL | 画面 | 言うこと |
| --- | --- | --- | --- |
| 0 | `/login` | ログイン | 上のデモ用アカウントでログインする（20秒） |
| 1 | `/products/2` | PRICO メルティリップ 03 | 「これ、もう持っています」。ポーチの LUMINA と中身ほぼ同じ・ほぼ同じ色で、**¥2,820 高い**方を持っている |
| 2 | `/products/13` | LUMINA スキンフィットファンデーション 200 | 「同じ処方でこっちが **¥4,720 安い**」（PRICO カバーリキッド） |
| 3 | `/products/36` | PRICO 9色アイパレット 01 | 「9色すべて手持ちで再現できる」。パレットは色単位で突き合わせている |
| 4 | `/products/15` | mode noir セラムファンデーション 300 | サクラ判定。11件中5件を総合評価から外している（表示は消さない） |
| 5 | `/stash` | Myポーチ | 「買い足しは提案しない」。手持ち5点だけでメイクを組む（リップ2本の被りもそのまま見せる） |

投入後にこの導線が生きているかは以下で確認する（CI でも走る）。

```bash
npm run smoke:demo
```

被り・安い代替・色カバレッジ・サクラ除外のいずれかが空振りしたら、`scripts/demo_smoke.sql` が
失敗して落ちる。デモ用アカウントとポーチの中身は `scripts/generate_seed.py` の `DEMO_*` で決めている。
...
npm run typecheck
npm run build
npm run smoke:demo    # supabase 起動 + db:reset 済みの状態で
```
