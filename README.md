...
| ポーチの公開範囲 | `/settings`。既定は非公開。全体 / リンクを知っている人だけ / 非公開の3段階＋公開プレビュー。説明は `/privacy` |
...
所持コスメは購買履歴に近い情報なので、既定は非公開。公開範囲は「全体に公開 / リンクを知っている人だけ / 非公開」の3段階で、`/settings` のプレビューで公開後の見え方を確認してから切り替える。何が公開されるかは `/privacy` に書いてある。

- 全体に公開: `user_items` の RLS が `profiles.stash_visibility = 'public'` のときだけ他人の読み取りを許す。
- リンクを知っている人だけ: 共有トークンを本人しか読めない `profile_share_tokens` に置き、`shared_stash_product_ids`（security definer）で照合する。リンクは作り直せる。
- 非公開: 本人だけ。
