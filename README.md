...
「同じ処方でこっちが安い」を出すので、価格の根拠を画面に添える。

- `products.price_source`（どこの価格か） / `products.price_checked_at`（いつ時点か）を価格の下に出す。
- 容量あたりの単価（`¥/g`・`¥/mL`）を必ず併記する。容量が未登録なら「容量未登録」と出す。
- 表示は参考価格で、販売店の実売価格とは異なる。KAWANAI は販売リンク・アフィリエイトを持たない（文言は `src/lib/price.ts` の `PRICE_DISCLAIMER` に集約）。
