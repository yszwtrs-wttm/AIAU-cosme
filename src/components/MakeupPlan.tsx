...
export default function MakeupPlan({ products }: { products: Product[] }) {
  ...
  return (
    <section className="space-y-3 rounded-2xl border border-ink-200 bg-surface p-4">
      ...
      <div className="flex flex-wrap gap-2">
        {PRESETS.map((p) => (
          <button
            ...
            type="button"
            onClick={() => run(p)}
            className={`rounded-full border px-3 py-1 text-sm ${request === p ? "border-strong bg-strong text-white" : "border-ink-200"}`}
          >
            {p}
          </button>
        ))}
      ...
      >
        ...
        <button
          type="submit"
          disabled={pending}
          className="rounded-lg bg-strong px-3 py-2 text-sm text-white disabled:opacity-50"
        >
          {pending ? "考え中…" : "組む"}
        </button>
      </form>
      ...
      {plan && (
        <div className="space-y-2 rounded-xl bg-ink-50 p-3">
          ...
          <p className="text-sm text-emerald-700 dark:text-emerald-300">{plan.note}</p>
          <p className="text-xs text-ink-400">
