...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <Link
      ...
      className="group flex gap-3 rounded-2xl border border-ink-200 bg-surface p-3 transition "
    >
      <ProductThumb
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5 text-[11px] text-ink-400">
          ...
          <span className="rounded-full bg-brand-50 px-1.5 py-0.5 text-brand-fg">
            {CATEGORY_LABEL[product.category]}
          </span>
          {product.is_mens && (
            <span className="rounded-full bg-strong px-1.5 py-0.5 text-[10px] text-white">MEN</span>
          )}
        </div>
