...
export default async function UserPage({ params }: { params: Promise<{ handle: string }> }) {
  ...
  return (
    <div className="space-y-6">
      <section className="flex flex-wrap items-center gap-4 rounded-xl border border-ink-200 bg-ink-0 p-6">
        <Avatar
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-xs text-ink-500">@{profile.handle}</div>
      ...
      <section className="space-y-3">
        ...
        {(reviews ?? []).length === 0 ? (
          ...
          <ul className="space-y-2">
            {(reviews ?? []).map((r) => (
              <li key={r.id} className="rounded-xl border border-ink-200 bg-ink-0 p-4">
                <div className="text-xs text-ink-500">{r.products?.brands?.name}</div>
